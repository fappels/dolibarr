...
jQuery(function () {
	var dialog, form,
		barcodeEl = jQuery("#scan_barcode"),
		warehouse = jQuery("#scan_warehouse"),
		qty = jQuery("#scan_qty");
	...
	form = dialog.find( "form" ).on( "submit", function( event ) {
		...
		findIndexFromBarcode(barcodeEl, parseFloat(qty.val()), warehouse.val());
	});
...
function findIndexFromBarcode(barcodeEl, qty, warehouse) {
	var lines = jQuery(":input[id^='barcode']"),
		scanResult = jQuery("#scan_result"),
		row, index, found = false;

	if (lines.length > 0) {
		lines.each(function (index, line) {
			if (!found) {
				if (line.value == barcodeEl.val()) {
					row = line.id.match(/_([0-9]*)/);
					index = line.id.match(/_([0-9]*)$/);
					incrementQty(index[1], row[1], qty);
					setWarehouse(index[1], row[1], warehouse);
					scanResult.val("Received OK");
					found = true;
				} else {
					scanResult.val("Not Found");
				}
			}
		});
	} else {
...
function incrementQty (index, row, qty) 
{
	var qtyEl = jQuery( "#qty_" + row + "_" + index),
		currentQty;

		qty ? null : qty=1; //default +1
		if (qtyEl) {
			currentQty = parseFloat(qtyEl.val());
			qtyEl.val(currentQty + qty);
		}
}

function setWarehouse (index, row, warehouse) 
{
	var entrepotEl = jQuery( "#entrepot_" + row + "_" + index),
		currentQty;

		warehouse ? null : warehouse=1; //default warehouse id 1
		if (entrepotEl) {
			entrepotEl.val(warehouse).change();
		}
}
